...
//    require('vendor/autoload.php');
    ...
    class NewsletterToKindle
    {
        protected $kindle_email;
        protected $imap_email;
        protected $imap_host;
        protected $imap_port;
        protected $imap_dir;
        protected $imap_user;
        ...
        function __construct($kindle_email, $imap_email, $imap_user, $imap_pass, $imap_host, $imap_port, $imap_dir, $debug = false) 
        {
            $this->kindle_email = $kindle_email;
            $this->imap_email   = $imap_email;
            $this->imap_user    = $imap_user;
            $this->imap_pass    = $imap_pass;
            $this->imap_host    = $imap_host;
            $this->imap_port    = $imap_port;
            $this->imap_dir     = $imap_dir;
        ...
        private function sendToKindle()
        {
            ...
            try 
            {
                ...
                //Attachments
                $mail->AddStringAttachment( $this->pdf_document->Output('', 'S') , 'newsletter_'.str_replace(' ', '_', strtolower($this->mail_subject)).'.pdf', 'base64', 'application/pdf');
